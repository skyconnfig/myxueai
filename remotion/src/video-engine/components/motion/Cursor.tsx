...
function buildKeyframes(steps: UiStep[], durationSec: number) {

  const sorted = [...steps].sort((a, b) => a.at - b.at)

  const keyframes: Array<{ at: number; x: number; y: number }> = [{ at: 0, x: 0.5, y: 0.5 }]
  ...
  for (const step of sorted) {
    ...
    if (step.x != null && step.y != null) {
      ...
      keyframes.push({ at: step.at, x: step.x, y: step.y })
...
function getCursorPosition(steps: UiStep[], frame: number, fps: number, durationInFrames: number) {
  ...
  const keyframes = buildKeyframes(steps, durationSec)
  ...
  for (let i = 0; i < keyframes.length - 1; i++) {
    ...
    if (timeSec >= a.at && timeSec <= b.at) {
      ...
      x = interpolate(t, [0, 1], [a.x, b.x], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })

      y = interpolate(t, [0, 1], [a.y, b.y], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })
